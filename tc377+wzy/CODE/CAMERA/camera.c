...
float Camera_scan(void)
{
   ...
   // gray_to_ori(Image_Sobel[ROW][COL]); //把120*128的灰度图压缩到64*128
...
{
	g_weight_sum = 0;
	for (uint16 y = SEARCH_BOT; y >= SEARCH_TOP; y -= SEARCH_STEP_Y)//91个
	{
		//g_err_weight[y] = 1;
...
// 快速OTSU最佳阈值（将原图像压缩10倍，这样时间短，可以每帧都计算一次）
...
uint16 image_find_whiteline()
{
	...
	// 计算 y 轴累加
 	for (uint16 x = SEARCH_LEFT; x <= SEARCH_RIGHT; x += DOWNSAMPLE_X)
 	{
 		uint16* sum = &y_sum[(x - SEARCH_LEFT) / DOWNSAMPLE_X];//DOWNSAMPLE_X:10 缩小10倍
...
uint16 image_fastsearch_leftline()
{
	...
	uint16 last_x = g_mid_x;//起源时的起始搜索点，从最长白列中线开始
	...
	while (y >= SEARCH_TOP)
	{
		// 动态阈值，近处阈值大，远处阈值小（因为远处的点灰度值普遍较小）
		...
		else
		{
			times++;
			// 第一次寻找根据之前的结果
			if (times == 1)//第一次搜线没搜到，继续第二次，并且已经搜过的线不用再搜
			{
				x = last_x + margin;
				end_x = SEARCH_LEFT;
			}
...
// 直线分析曲率
/*作用：用于圆环，当搜索到一边曲率特别大，一边曲率特别小，即一边是环，一边是直线，判定为圆环*/
